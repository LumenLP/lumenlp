...
IFS='|' read -r source_event_id leader pool kind amounts quote_stroops ledger session_id db_contract_session_id op_status scaled_quote_xlm scaled_amounts <<< "$row"
...
if [[ "$kind" != "deposit" && "$kind" != "withdraw" ]]; then
  echo "Refusing to execute '$kind': claim_token resolution is not yet wired into the recorder payload" >&2
  exit 1
fi
...
for command in record_leader_event execute_aquarius_standard_op; do

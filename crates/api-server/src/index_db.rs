...
impl IndexDb {
    ...
    fn migrate(&self) -> Result<()> {
        ...
        self.ensure_column("copy_sessions", "contract_address", "TEXT")?;
    ...
    pub fn create_wallet_auth_token(&self, token_hash: &str, address: &str, expires_at: i64, now: i64) -> Result<()> {
        self.conn.execute(
    ...
    pub fn create_copy_session(
        ...
        self.conn.execute(
            ...
            INSERT INTO copy_sessions (
              id, contract_address, contract_session_id, follower_address, leader_address, coefficient, status,
    ...
    pub fn list_copy_sessions(&self, follower: &str) -> Result<Vec<CopySessionRow>> {
        let mut stmt = self.conn.prepare(
            r#"
            SELECT id, contract_address, contract_session_id, follower_address, leader_address, coefficient, status,
    ...
    pub fn update_copy_session(
        ...
        if let Some(value) = contract_address {
            row.contract_address = Some(value.to_string());
        }
        ...
        self.conn.execute(
            ...
            SET status = ?2, coefficient = ?3, watermark_ts = ?4,
                ...
                contract_address = ?7, contract_session_id = ?8, updated_at = ?9
            WHERE id = ?1
            "#,
            params![
                id,
...
fn map_copy_session_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<CopySessionRow> {
    let allowed_pools_json: String = row.get(8)?;
    Ok(CopySessionRow {
        id: row.get(0)?,
        contract_address: row.get(1)?,
        contract_session_id: row.get(2)?,
        follower_address: row.get(3)?,
        leader_address: row.get(4)?,
        coefficient: row.get(5)?,
        status: row.get(6)?,
        include_claims: row.get::<_, i64>(7)? != 0,
        allowed_pools: serde_json::from_str(&allowed_pools_json).unwrap_or_default(),
        max_per_op_quote_xlm: row.get(9)?,
        max_daily_quote_xlm: row.get(10)?,
        expires_at: row.get(11)?,
        cursor_ts: row.get(12)?,
        watermark_ts: row.get(13)?,
        watermark_event_id: row.get(14)?,
...
mod tests {
    ...
    fn wallet_auth_challenges_are_single_use_and_tokens_expire() {
        ...
        assert_eq!(
            db.wallet_auth_token_address("hash", 199).unwrap().as_deref(),
            Some("GADDRESS")
        );
    ...
    fn recorder_claim_token_is_available_for_prepare() {
        ...
        assert_eq!(
            db.recorder_claim_token("evt-claim").unwrap().as_deref(),
            Some("CREWARD")
        );
    ...
    fn insert_copy_op_is_idempotent() {
        let db = test_db();
        let session = db
            .create_copy_session("GFOLLOWER", "GLEADER", 0.5, false, &[], 0.0, 0.0, None, None, None)
            .unwrap();
        let op = CopyOpRow {
            id: "op-1".into(),
    ...
    fn create_copy_session_pauses_prior_active_pair() {
        let db = test_db();
        let first = db
            .create_copy_session("GFOLLOWER", "GLEADER", 1.0, false, &[], 0.0, 0.0, None, None, None)
            .unwrap();
        ...
        let stored = db.get_copy_session(&second.id).unwrap().unwrap();
        assert_eq!(stored.contract_address.as_deref(), Some("CPOLICY"));
        assert_eq!(stored.contract_session_id, Some(42));
